...
float my_asin(float x)
{
    ...
    ans=x;
    u=x;
    while (myabs_f(u)>=1.0e-7){
            if (myabs_f(u)<1.0e-7) break;
...
// Sample usage:
//-------------------------------------------------------------------------------------------------------------------
void init_arcsin_table()
{ 
    int i;
    for (i = 0; i < TABLE_SIZE; i++) 
    {
        double x = -1.0 + 2.0 * i / (TABLE_SIZE - 1);
        arcsin_table[i] = asin(x);
    }
}
//-------------------------------------------------------------------------------------------------------------------
// @brief       查表法计算arcsin
// @param       double x arcsin(x)
// @return      double 返回查表得到的值
// Sample usage:
//-------------------------------------------------------------------------------------------------------------------
double arcsin_lookup(double x) 
{
    double index;
    double result;
    int i;
    ...
    if (x >= 1.0)
        return PI / 2;
    index = (x + 1.0) * 0.5 * (TABLE_SIZE - 1);
    i = (int)index;
    frac = index - i;
    result = arcsin_table[i] + frac * (arcsin_table[i + 1] - arcsin_table[i]);
    return result;
}
